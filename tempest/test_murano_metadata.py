...
class SanityMuranoTest(base.MuranoMeta):
    ...
    @attr(type='negative')
    def test_get_list_metadata_objects_without_token(self):
        resp = self.get_list_metadata_objects_without_token("ui")
        assert resp.status_code == 401

    @attr(type='negative')
    def test_get_list_ui_definitions_without_token(self):
        resp = self.get_ui_definitions_without_token()
        assert resp.status_code == 401
    ...
    @attr(type='negative')
    def test_get_conductor_metadata_without_token(self):
        resp = self.get_conductor_metadata_without_token()
        assert resp.status_code == 401
    ...
    @attr(type='negative')
    def test_create_directory_without_token(self):
        resp = self.create_directory_without_token("scripts", "testdir")
        assert resp.status_code == 401

    @attr(type='negative')
    def test_delete_directory_without_token(self):
        self.create_directory("scripts/", "testdir")
        resp =\
            self.delete_metadata_obj_or_folder_without_token("scripts/testdir")
    ...
    @attr(type='smoke')
    def test_upload_metadata_object_without_token(self):
        resp = self.upload_metadata_object_without_token(path='manifests')
        assert resp.status_code == 401
    ...
    @attr(type='negative')
    def test_get_metadata_object_without_token(self):
        self.upload_metadata_object(path="workflows")
        resp = self.get_metadata_object_without_token("workflows/testfile.txt")
        self.delete_metadata_obj_or_folder("workflows/testfile.txt")
        assert resp.status_code == 401
    ...
    def test_create_service_without_token(self):
        resp = self.create_new_service_without_token('test')
        assert resp.status_code == 401

    @attr(type='negative')
    def test_delete_service_without_token(self):
        self.create_new_service('test')
        resp = self.delete_service_without_token('test')
    ...
    def test_update_service_without_token(self):
        self.create_new_service('test')
        resp = self.update_new_service_without_token('test')
        self.delete_service('test')
        assert resp.status_code == 401
