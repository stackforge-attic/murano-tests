...
class UITestCase(testtools.TestCase):
    ...
    def create_iisfarm_service(self, service_name):
        ...
        self.select_from_list('service_choice-service', 'Internet Information Services Web Farm')
        Next = self.elements.get('button', 'Next')
        self.driver.find_element_by_xpath(Next).click()

        self.find_clean_send(by.By.ID, 'id_webServerFarm-0-name', service_name)
        self.find_clean_send(by.By.ID, 'id_webServerFarm-0-adminPassword', 'P@ssw0rd')
        self.find_clean_send(by.By.ID, 'id_webServerFarm-0-adminPassword-clone', 'P@ssw0rd')
        Next = self.elements.get('button', 'Next2')
        self.driver.find_element_by_xpath(Next).click()
    ...
    def create_aspfarm_service(self, service_name):
        ...
        self.select_from_list('service_choice-service', 'ASP.NET Application Web Farm')
        Next = self.elements.get('button', 'Next')
        self.driver.find_element_by_xpath(Next).click()

        self.find_clean_send(by.By.ID, 'id_aspNetAppFarm-0-name', service_name)
        self.find_clean_send(by.By.ID, 'id_aspNetAppFarm-0-adminPassword', 'P@ssw0rd')
        self.find_clean_send(by.By.ID, 'id_aspNetAppFarm-0-adminPassword-clone', 'P@ssw0rd')
        self.find_clean_send(by.By.ID, 'id_aspNetAppFarm-0-repository', self.asp_git_repository)
        Next = self.elements.get('button', 'Next2')
        self.driver.find_element_by_xpath(Next).click()
