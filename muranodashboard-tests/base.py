...
class UITestCase(testtools.TestCase):
    ...
    def setUpClass(cls):
        ...
        for i in keystone_client.services.list():
            if i.name == 'glance':
                glance_id = i.id
                break
        for i in keystone_client.endpoints.list():
            if i.service_id == glance_id:
                glance_endpoint = i.publicurl
                break
    ...
    def get_image_name(self, name, glance):
